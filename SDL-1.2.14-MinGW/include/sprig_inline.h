...
static inline void SPG_GetPointsFromArrays(Uint16 n, SPG_Point* pointsBuffer, Sint16* x, Sint16* y)
{
    Uint16 i = 0;
    for(; i < n; i++)
    {
        pointsBuffer[i].x = x[i];
        pointsBuffer[i].y = y[i];
    }
}

static inline void SPG_GetArraysFromPoints(Uint16 n, Sint16* x, Sint16* y, SPG_Point* pointsBuffer)
{
    Uint16 i = 0;
    for(; i < n; i++)
    {
        x[i] = pointsBuffer[i].x;
        y[i] = pointsBuffer[i].y;
    }
}
